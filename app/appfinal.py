...
#             df_status = pd.read_sql_query(
#                 """
#                 SELECT a.*, ROW_NUMBER() OVER (ORDER BY CURR_HEAT_NO, ROWNUM) AS explicit_rowid
...
#             df_material = pd.read_sql_query(
#                 """
#                 SELECT a.*, ROW_NUMBER() OVER (ORDER BY HEATNO, ROWNUM) AS explicit_rowid
...
# def get_data_from_online_db(heat_number: str, addition_type: str) -> pd.Series:
#     """
#     Fetch heat data from Oracle DB without using explicit_rowid.
#     """
#     import oracledb
#     import pandas as pd
#     import streamlit as st
...
#             )
#             df_material = pd.read_sql_query(
#                 "SELECT * FROM CONVERTER_MATERIAL", oracle_conn
#             )
#             df_analysis = pd.read_sql_query("SELECT * FROM SMS3_ANALYSIS1", oracle_conn)
...
#             # Filter latest status and material by CURR_HEAT_NO / HEATNO
#             status_row = df_status[df_status["CURR_HEAT_NO"] == heat_number]
#             if status_row.empty:
#                 return None
#             status_row = status_row.tail(1)  # latest row

#             material_row = df_material[df_material["HEATNO"] == heat_number]
#             if material_row.empty:
#                 return None
#             material_row = material_row.tail(1)
...
#             # Merge all
#             final_df = pd.merge(
#                 status_row, analysis_row,
#                 left_on="CURR_HEAT_NO", right_on="SID1", how="left"
#             )
#             final_df = pd.merge(
#                 final_df, material_row,
#                 left_on="CURR_HEAT_NO", right_on="HEATNO", how="left"
#             )

#             return final_df.iloc[0]  # single row as Series
...
#         # --- Fetch top 5 rows from a table ---
#         query = "SELECT * FROM CONVERTER_STATUS_M FETCH FIRST 5 ROWS ONLY"
#         df = pd.read_sql(query, conn)
        
#         print("✅ Top 5 rows fetched successfully:")
#         print(df)
